...
{ disable();
  ...
  LOG("init and %u %u\r\n",sizeof(class_t),sizeof(run_mode_t));
	WRITE(pinState, MODE_IDLE, uint16_t, 0);
  WRITE(count, 0, unsigned, 0);
  WRITE(seed, 1, unsigned, 0);
...
void task_sample()
{
  ...
  TX_BEGIN
  disable();capybara_transition(0);enable();
  if(complete && (TX_READ(samplesInWindow, unsigned) != test_siw)) {
...
void task_warmup()
{
  ...
  uint16_t temp = READ(mode, uint16_t);
	LOG("warmup, %u\r\n", temp);
  if(complete && (READ(samplesInWindow, unsigned) != test_siw)) {
