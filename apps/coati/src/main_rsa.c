...
{ disable();
  ...
  /*for( i = 0; i < 100; i++) {
    ...
    WRITE(modulus[i], pubkey.n[i], digit_t, 0);
	}

	LOG("init: out exp\r\n");
  WRITE(message_length, locmessage_length, unsigned, 0);
	WRITE(block_offset, 0, unsigned, 0);
	WRITE(cyphertext_len, 0, unsigned, 0);
  WRITE(iter, 0, unsigned, 0);

	LOG("init: done %u %u\r\n",READ(message_length,unsigned),
                          READ(block_offset, unsigned));
...
void task_loop_entry() {
  ...
  if(TX_READ(ready,unsigned) == 0) {
    WAIT_TIMER_START;
    if(complete && (TX_READ(ready, unsigned) != test_ready)) {
      P1OUT |= BIT5;
      P1DIR |= BIT5;
      P1OUT &= ~BIT5;
    ...
    __delay_cycles(4000);
    NI_TX_END_TRANSITION_TO(task_loop_entry);
  }
  WAIT_TIMER_STOP;
  complete = 0;
...
void task_mult()
{
  ...
  /*for(int i = 0; i < tsk_buf_level; i++) {
    LOG("%x ",(uint8_t)tsk_buf[i]);
  }
  LOG("\r\n");*/
  //LOG("buf_level: %u\r\n",tsk_buf_level);
  ...
  LOG("buf_level: %u\r\n",tsk_buf_level);*/
  temp++;
	TX_WRITE(digit, temp, digit_t, 0);
  //LOG("mult: digit=%u %u\r\n",TX_READ(digit, digit_t), temp);
  /*for(int i = 0; i < tsk_buf_level; i++) {
    LOG("%x ",(uint8_t)tsk_buf[i]);
  }
  LOG("\r\n");
