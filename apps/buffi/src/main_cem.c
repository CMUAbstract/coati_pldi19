...
// processing.
// This is the modified version of the CoatiB app that runs this code- it's for
// the next iteration of the eval.
...
{ disable();
  ...
  P1OUT &= ~BIT5;
	LOG("init\r\n");
	NI_WRITE(parent_next, 0, unsigned, 0);
  NI_WRITE(parent, 0, unsigned, 0);
  NI_WRITE(child, 0, unsigned, 0);
  NI_WRITE(sibling, 0, unsigned, 0);
  // TODO: this var hasn't been initialized has it?
	LOG("init: start parent %u\r\n", READ(parent,unsigned));
	NI_WRITE(out_len, 0, unsigned, 0);
	NI_WRITE(letter, 0, unsigned, 0);
  NI_WRITE(sample, 0, unsigned, 0);
	NI_WRITE(prev_sample, 0, unsigned, 0);
	NI_WRITE(letter_idx, 0, unsigned, 0);
	NI_WRITE(sample_count, 1, unsigned, 0);
  NI_WRITE(iter, 0, unsigned, 0);
  NI_WRITE(last_iter, 0xFF, unsigned, 0);
  NI_WRITE(new_samp, 0, unsigned, 0);
  NI_WRITE(ready, 0, unsigned, 0);
  NI_WRITE(node_count, 0, unsigned, 0);
	NI_TRANSITION_TO(task_init_dict);
}
...
void task_done()
{
  PRINTF("Custom finish: %u events %u missed\r\n", _numEvents,
