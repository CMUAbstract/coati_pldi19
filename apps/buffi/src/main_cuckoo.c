// This is the one we'll use for the new eval
#include <msp430.h>
#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>
...
{ TX_BEGIN
  ...
  enable();
	...
	NI_TX_WRITE(insert_count,0,value_t,0);
	NI_TX_WRITE(insert_tsk_count,0,value_t,0);
	NI_TX_WRITE(insert_ev_count,0,value_t,0);
	NI_TX_WRITE(lookup_count,0,value_t,0);
	NI_TX_WRITE(inserted_count,0,value_t,0);
	NI_TX_WRITE(member_count,0,value_t,0);
	NI_TX_WRITE(key,READ(init_key, value_t),value_t,0);
	LOG("Starting inserts!! %x\r\n", TX_READ(key, value_t));
	NI_TX_TRANSITION_TO(task_generate_key_in_tx);
}
...
void task_done()
{
    ...
    disable();
    capybara_transition(0);
    PRINTF("buff cuckoo final: %u events %u missed\r\n", _numEvents,
